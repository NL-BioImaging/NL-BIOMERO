#!/bin/bash
#SBATCH --job-name=aggregates_measurements
...
echo "Running Aggregates Measurements Job w/ $IMAGE_PATH | $SINGULARITY_IMAGE | $DATA_PATH"
...
singularity run "$IMAGE_PATH/$SINGULARITY_IMAGE" \
    ...
    --aggregates_mask_suffix "${AGGREGATES_MASK_SUFFIX:-_Aggregates_Mask}" \
