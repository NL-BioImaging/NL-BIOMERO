...
The explicit `1` CPU and `2G` memory values above are portable defaults for
the bundled local Slurm demos. On another cluster, leaving a dedicated image
pull resource blank inherits the matching global setting, such as
`sbatch_cpus-per-task` or `sbatch_mem`; if neither is set, Slurm applies its
scheduler default.
...
BIOMERO library defaults remain backward compatible. The NL-BIOMERO local-demo
template deliberately enables scheduler-native image initialization so container
builds do not consume the Slurm controller/login node. Its resource values must
fit both linked local Compose clusters; real HPC deployments belong in their
site-specific repositories and should override these values there.
...
environment:
  ...
  # Portable values for the local CPU and GPU Compose demos.
  BIOMERO_PULL_CPUS: "1"
  BIOMERO_PULL_MEM: "2G"
  BIOMERO_PULL_CONCURRENCY: "2"
  # Do not set BIOMERO_PULL_TIME or BIOMERO_PULL_PARTITION here: the local
  # demos use unlimited time and have different default partition names.
